...
ggplot(county_data)+
  ...
  geom_point()
# this one above does not change the color of the points on the scatterplot.
...
region <- county_data %>%
  filter(NAME %in% c("Howard County, Maryland",
...
reliability_table <- county_data %>%
  mutate(
    ...
    NAME = str_remove(NAME, " County, Maryland")
  ) %>%
  arrange(desc(cv)) %>%
...
reliability_table %>%
  ...
  tab_header(
    ...
    subtitle = "Maryland counties, ACS 2023 5-year estimates"
  ) %>%
  tab_source_note(
